...
> | 版本 | 日期 | 变更摘要 |
> |------|------|---------|
> | **v7.2.2** | 2026-05-31 | **Bug fix** — ① 时间线网页预览内容被裁剪：≥6 条目自动启用 compact 模式（缩小点/字/间距），≥8 条目隐藏 desc，多条目时改用 `justify-start` 防止双侧截断；② 下载 PPTX 时间线超出页面：修复 rowH 算法（限制在 3.6″ 可用高度内，row < 0.65″ 时自动省略 desc）；③ 下载 PPTX 与网页预览排版不一致：新增 6 个 v7 layout_family 专属渲染器（`process_flow` 编号步骤链、`matrix_compare` 表格对比、`dense_report` 双列列表、`card_cluster` 主卡+辅卡、`split_argument` 60/40 不对称分栏、`evidence_board` 主张+卡片+来源条、`before_after` 玫瑰红/翠绿面板），下载器现在与浏览器预览保持一致 |
